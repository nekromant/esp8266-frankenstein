...
static  __attribute__ ((section(".iram0.text"))) void commit_handler(void* a)
{
	...
	console_printf("\nComitting update, %d sectors %d bytes\n", numsect, u->numbytes);
...
static int  do_tftp_info(int argc, const char* const* argv)
{
	console_printf("Flash area start @ %p len %p bytes\n",
		fr_fs_flash_offset(), CONFIG_ENV_OFFSET - fr_fs_flash_offset());
	return 0;
}

CONSOLE_CMD(tftp_info, 1, 1,
	    do_tftp_info, NULL, NULL,
	    "Print info about update possibilities"
	    HELPSTR_NEWLINE ""
	);
