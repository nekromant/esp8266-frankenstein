...
void ICACHE_FLASH_ATTR httpdStartResponse(HttpdConnData *conn, int code) {
	...
	espconn_send(conn->conn, (uint8 *)buff, l);
}
...
//		os_printf("%s == %s?\n", builtInUrls[i].url, conn->url);
	...
	espconn_send(conn->conn, (uint8 *)httpNotFoundHeader, os_strlen(httpNotFoundHeader));
	conn->cgi=NULL; //mark for destruction
}
...
static void ICACHE_FLASH_ATTR httpdWrDoneCb(void *arg) {
	console_printf("send done\n");
}
...
static void ICACHE_FLASH_ATTR httpdConnectCb(void *arg) {
	...
	espconn_set_opt(conn,  ESPCONN_KEEPALIVE  | ESPCONN_COPY | ESPCONN_NODELAY);
...
void ICACHE_FLASH_ATTR httpdInit(HttpdBuiltInUrl *fixedUrls, int port) {
	...
	espconn_set_opt(&httpdConn,  ESPCONN_KEEPALIVE  | ESPCONN_COPY | ESPCONN_NODELAY);
	espconn_tcp_set_max_con(MAX_CONN);	
}
