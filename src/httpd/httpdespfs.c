...
int ICACHE_FLASH_ATTR cgiEspFsHook(HttpdConnData *connData) {
	...
	len=espFsRead(file, buff, HTTPD_BUF_SIZE);
	int ret;
	if (len>0) {
		ret = espconn_send(connData->conn, (uint8 *)buff, len);
	}
	if (len!=HTTPD_BUF_SIZE) {
		//We're done.
		espFsClose(file);
		return HTTPD_CGI_DONE;
	} else {
...
int ICACHE_FLASH_ATTR cgiEspFsTemplate(HttpdConnData *connData) {
	...
	len=espFsRead(tpd->file, buff, HTTPD_BUF_SIZE);
	if (len>0) {
		sp=0;
		e=buff;
		for (x=0; x<len; x++) {
